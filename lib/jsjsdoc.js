...
exports.format = function(doc_object, options, callback){
    ...
    function formatDetail(detail, add_line_prefix){
        add_line_prefix = add_line_prefix || "";
    
        return add_line_prefix + detail.split('\n').join('\n' + add_line_prefix);
    }
    ...
    function formatConstructor(object, level){
        ...
        if('$brief' in object)
            doc += object.$brief + '\n\n';
        if('$detail' in object)
            doc += formatDetail(object.$detail) + '\n\n';
    ...
    function formatParameters(parameters, level){
        ...
        if('$detail' in parameters)
            doc += formatDetail(parameters.$detail) + '\n\n';
    ...
    function formatNamespace(object, level){
        var doc = '';
        for(var name in object){
            if(!nameIsInternal(name))
                doc += formatObject(object[name], name, 0);
        }
        return doc;
    }
    ...
    function formatObject(object, name, level){
        ...
        if(type == 'class'){
            ...
            doc += formatNamespace(object, level+1);
        }else{
    ...
    var formatted = '';
    try{
        formatted = formatNamespace(doc_object, 0);
    }catch(err){
        return callback(err);
    }
